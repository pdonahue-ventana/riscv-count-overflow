...
This "count overflow interrupt request" signal is treated as a standard edge-triggered local
...
interrupts. LCOFIP is read-write in mip. It is set on the occurrence of a local
count overflow interrupt request.
If the Sscofpmf extension is not implemented, these LCOFIP and
...
the interrupt-pending and interrupt-enable bits for local count overflow
interrupts. LCOFIP is read-write in sip. It is set on the occurrence of a local
count overflow interrupt request.
If the Sscofpmf extension is not implemented, these LCOFIP and
LCOFIE bits are hardwired to zeros.
